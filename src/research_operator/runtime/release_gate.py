...
def check_watch_surface(repo_root: Path) -> GateCheck:
    cli_path = repo_root / "src" / "research_operator" / "cli.py"
    text = cli_path.read_text(encoding="utf-8")
    required_tokens = ["watch_create", "watch_run", "watch_run_all", "watch_inspect", "watch_delivery_manifest", "watch_set_enabled", "--enabled-only", "--disabled-only", "watch_list"]
    missing = [token for token in required_tokens if token not in text]
    passed = not missing
...
def check_api_surface(repo_root: Path) -> GateCheck:
    ...
    required_tokens = ["/doctor", "/runs/{run_id}/verify", "/runs/{run_id}/deliverables", "/runs/{run_id}/delivery-manifest", "/runs/{run_id}/quality", "enabled: bool | None = None", "/watches/{watch_id}", "/watches/{watch_id}/delivery-manifest", "@app.patch(\"/watches/{watch_id}\")"]
    missing = [token for token in required_tokens if token not in text]
