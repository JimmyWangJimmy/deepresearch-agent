...
def check_cli_surface(repo_root: Path) -> GateCheck:
    ...
    required_tokens = ["def run(", "def verify(", "def export(", "format == \"all\"", "--task-type", "--task-contains", "--has-warnings/--no-has-warnings", "--min-quality-score", "--max-quality-score", "--min-source-count", "--max-source-count", "--min-event-count", "--max-event-count", "--limit", "def providers(", "def runs(", "def quality(", "def doctor("]
...
def check_api_surface(repo_root: Path) -> GateCheck:
    ...
    required_tokens = ["/doctor", "task_type: TaskType | None = None", "task_contains: str | None = None", "has_warnings: bool | None = None", "min_quality_score: float | None = None", "max_quality_score: float | None = None", "min_source_count: int | None = None", "max_source_count: int | None = None", "min_event_count: int | None = None", "max_event_count: int | None = None", "limit: int | None = None", "/runs/{run_id}/verify", "/runs/{run_id}/deliverables", "/runs/{run_id}/delivery-manifest", "/runs/{run_id}/quality", "enabled: bool | None = None", "/watches/{watch_id}", "/watches/{watch_id}/delivery-manifest", "@app.patch(\"/watches/{watch_id}\")", "@app.delete(\"/watches/{watch_id}\")"]
    missing = [token for token in required_tokens if token not in text]
