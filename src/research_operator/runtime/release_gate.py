...
def check_cli_surface(repo_root: Path) -> GateCheck:
    ...
    required_tokens = ["def run(", "def verify(", "def export(", "format == \"all\"", "def providers(", "def runs(", "def quality(", "def doctor("]
...
def check_api_surface(repo_root: Path) -> GateCheck:
    ...
    required_tokens = ["/doctor", "/runs/{run_id}/verify", "/runs/{run_id}/deliverables", "/runs/{run_id}/delivery-manifest", "/runs/{run_id}/quality", "enabled: bool | None = None", "/watches/{watch_id}", "/watches/{watch_id}/delivery-manifest", "@app.patch(\"/watches/{watch_id}\")", "@app.delete(\"/watches/{watch_id}\")"]
    missing = [token for token in required_tokens if token not in text]
