...
def test_runs_endpoint_filters_by_task_type_and_limit(tmp_path):
    ...
    eventful = client.get(
        "/runs",
        params={"artifacts_dir": str(tmp_path), "min_event_count": 1},
    )
    assert eventful.status_code == 200
    eventful_payload = eventful.json()["runs"]
...
def test_watch_lifecycle_via_api(tmp_path):
    ...
    assert status_summary_payload["enabled_rate"] == 1.0
